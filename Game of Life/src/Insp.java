...
public class Insp implements Initializable {
...
//		addBlack();
	...
	//pr�vde � lage gol-regler ift naboer, fikk noen exception errors. regner med at det har noe � gj�re med at celler som befinner seg
	// i endene av gridden ikke finner naboer. eller at jeg m� forandre loopen fra � s�ke gjennom samtlige celler i gridden og heller kun s�ke de
	// 8 n�rmeste.
...
//						board[i-1][j+1] + board[i-1][j]) > 3) {
//				
//				board[i][j] = 0; //cellen d�r av overpopulasjon
//				
//			} else if	((board[i-1][j-1] + board[i][j-1] + 
//						board[i+1][j-1] + board[i+1][j] +
//						board[i+1][j+1] + board[i][j+1] + 
//						board[i-1][j+1] + board[i-1][j]) < 3) {
//				
//				board[i][j] = 0; //cellen d�r av ensomhet
//				
//			}
//		}
//	}
...
//		plist = new ArrayList<Point>();
		drawBoard();
		clearCanvas(); //pr�ver � bli kvitt s�ppel som blir igjen i bakgrunnen.
		//dette gj�r bare at funksjonen starter blankt.
	}
